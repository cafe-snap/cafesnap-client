...
- [📝 구현 세부사항](#%F0%9F%93%9D-%EA%B5%AC%ED%98%84-%EC%84%B8%EB%B6%80%EC%82%AC%ED%95%AD)
  ...
  * [검색에 사용되는 영어 ➡ 한글 자동 변환 기능](#%EA%B2%80%EC%83%89%EC%97%90-%EC%82%AC%EC%9A%A9%EB%90%98%EB%8A%94-%EC%98%81%EC%96%B4-%E2%9E%A1-%ED%95%9C%EA%B8%80-%EC%9E%90%EB%8F%99-%EB%B3%80%ED%99%98-%EA%B8%B0%EB%8A%A5)
    + [1. 한글의 분리 : 초성, 중성, 종성](#1-%ED%95%9C%EA%B8%80%EC%9D%98-%EB%B6%84%EB%A6%AC--%EC%B4%88%EC%84%B1-%EC%A4%91%EC%84%B1-%EC%A2%85%EC%84%B1)
...
## 검색에 사용되는 영어 ➡ 한글 자동 변환 기능

키워드 검색 기능에 대해 사용자 편의성을 향상할 수 있는 요소를 고민하던 중 구현된 기능입니다. 키워드를 입력하기 위해 타이핑을 할 때, 종종 영타가 입력되는 경우가 있습니다. 이럴 때, 입력된 문자를 모두 다 지우고 한글을 다시 입력하는 게 아니라, 한글로 변환 시켜주면 어떨까? 라는 생각이 들었고 직접 구현하기 시작했습니다.

---

### 1. 한글의 분리 : 초성, 중성, 종성

가장 먼저 시작한 작업은 한글에 대한 분리 작업이었습니다. 한 문장을 이루는 구성 요소를 분리하다 보면 한 글자가 나오게 되고, 해당 글자는 2~3개의 자음, 모음으로 만들어져있습니다. 이 구성을 좀 더 자세히 분리해 보자면 한 글자의 시작을 담당하는 초성, 중간을 담당하는 중성, 글자의 끝을 알리는 종성, 이런 3개의 요소로 이루어져 있습니다.  
해당 3요소를 각각 배열로 상수화했고, 영문 키보드 입력과 한글 자음, 모음을  맵핑하는 객체 또한 정의했습니다. 이를 통해 영어 ➡ 한글 자동 변환 알고리즘을 구현할 기초적인 준비를 완료했습니다.
...
현재 동영상의 진행률을 표현할 때, 프로그레스바가 차오르는 듯한 애니메이션을 다음과 같이 구현했습니다.

<strong> 2-1. useRef 와 video 태그를 활용한 재생 길이의 퍼센트 변환 </strong>

진행률을 실시간으로 표기하기 위해서는 영상의 재생 길이를 알아내야 했습니다. 동영상과 관련된 정보는 video 태그가 제공하는 속성을 사용하여 얻을 수 있습니다. 현재 영상 시간을 제공해 주는 `currentTime` 과 영상의 총길이를 제공해 주는 `duration` 속성을 활용했습니다. 해당 속성값들을 토대로 길이를 퍼센트로 변환하는 함수를 작성했습니다.
...
팀 프로젝트가 끝난 이후 진행한 개인 프로젝트라 팀의 소중함이 더 크게 다가왔던 시간이었습니다. 팀으로 진행할 때는 내 생각에 대한 피드백이 즉각적으로 돌아왔고, 그로 인해 의사결정을 빠르게 바른 방향으로 내릴 수 있었습니다. 하지만, 이번 프로젝트는 처음 기획부터 마무리까지 모든 단계를 오롯이 나의 생각으로만 방향을 잡아가야 했기에 어려움이 컸었습니다.
